...
for (let i = 0; i < questions.length; i++) {
  document.getElementById("questionHolder").innerHTML += `<div class="question"> ${i + 1}. ${
    questions[i]
  } </div>`;
}

for (let i = 0; i < qualitatives.length; i++) {
  document.getElementById("qualitativeHolder").innerHTML += `<div class="question"> ${i + 1}. ${
    qualitatives[i]
  } </div>`;
}
